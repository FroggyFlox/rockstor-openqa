...
sub run {
    ...
    my $bootloader_timeout = get_var('REBOOT', 0) ? 30 : 60;

    # we first check whether the bootloader is visible
    my $match = assert_screen('kiwi_bootloader', timeout => $bootloader_timeout);
    if (get_var('UEFI') && !defined($match)) {
    ...
    # match
    # Do not use any PUBLISH_HDD_n vars for now to simplify testing
    ...
    #     send_key('ret');
    # }
    if (match_has_tag('kiwi_bootloader_boot_from_hdd')) {
            send_key('home');
            send_key_until_needlematch('kiwi_bootloader_install_selected', 'down', 10);
            send_key('ret');
        }
    ...
    if (match_has_tag('installation_screen')) {
        ...
        send_key_until_needlematch('en_US_selected', 'down');
        send_key('ret');
        assert_screen('keyboard_layout', 60);
        send_key_until_needlematch('us_keyboard_selected', 'down');
        send_key('ret');
        assert_screen('license_agreement', 60);
        send_key('ret');
        send_key('ret');
